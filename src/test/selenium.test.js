...
test('Hide Private Builds', async () => {
  ...
  expect(['http://localhost:9000', 'http://localhost:9000/']).toContain(
    currentUrl
  );
});
...
test('Make New Build', async () => {
  ...
  expect(
    await driver.findElement(By.css('input[name="name"]')).getAttribute('value')
  ).toBe(buildName);
  ...
  expect(
    (await driver.findElement(By.id('cooler')).findElements(By.css('*'))).length
  ).toBeGreaterThan(0);
  expect(
